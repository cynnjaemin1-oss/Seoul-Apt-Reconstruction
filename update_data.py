...
DISTRICT_CODES = {
    ...
    "성동구":"11200","용산구":"11170","마포구":"11440","노원구":"11350",
}

# ── 재건축 단지 검색 규칙 ──────────────────────────────
RECON_SEARCH = {
    "잠실장미":         {"include":["장미1","장미2"],                  "exclude":[], "area_range":(45, 145)},
    "잠실우성1·2·3차":  {"include":["우성아파트"],                     "exclude":["우성4차","가락우성","도곡우성"], "area_range":(75, 170)},
...
def update_my_home(mh):
    ...
    for i in range(MONTHS_BACK):
        ...
        for item in items:
            ...
            try:
                p = round(int(str(item.get("dealAmount","0")).replace(",","")) / 10000, 1)
            except:
                continue
            year  = str(item.get("dealYear","")).strip()
            month = str(item.get("dealMonth","")).strip().zfill(2)
